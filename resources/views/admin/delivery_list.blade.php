                                ...
                                <div class="table-responsive">
                                    <table class="table table-striped mb-0">
                                        ...
                                        @foreach($deliveries as $delivery)
                                            <tr>
                                                <th class="col-lg-1">
                                                    <img src="{{$delivery->showImage()}}" width="50" height="50"/>
                                                </th>
                                                <td class="col-lg-7">{{$delivery->name}}</td>
                                                <td class="col-lg-1">
                                                    {{--<div class="custom-control custom-checkbox checkbox-primary">
                                                        <input type="checkbox" class="custom-control-input" id="list_item_{{$delivery->id}}" @if($delivery->enabled) checked @endif >
                                                        <label class="custom-control-label" for="list_item_{{$delivery->id}}"></label>
                                                    </div>--}}
                                                ...
                                                <td class="col-lg-3 text-center">
                                                    @can('view', $delivery)
                                                        <a href="{{route('deliveries.edit', $delivery->id)}}" class="mr-25" data-toggle="tooltip" data-original-title="Edit"> <i class="icon-pencil"></i> </a>
                                                    @endcan
                                                    @can('delete', $delivery)
                                                        <a href="javascript:;" class="fn_list_del_it"  data-action="{{route('deliveries.destroy', ['id'=>$delivery->id])}}" data-toggle="tooltip" data-original-title="Delete"> <i class="icon-trash txt-danger"></i> </a>
                                                    @endcan
                                                </td>
                                            </tr>
                                        @endforeach
